...
class Save extends Action implements HttpPostActionInterface
{
    ...
    private function processSave(
        ...
        try {
            $manifestationId = isset($requestData['general'][ManifestationInterface::ID])
                ? (int)$requestData['general'][ManifestationInterface::ID]
                : null;
    ...
    private function processRedirectAfterSuccessSave(Redirect $resultRedirect, int $manifestationId)
    {
        if ($this->getRequest()->getParam('back')) {
            $resultRedirect->setPath('*/*/edit', [
                ManifestationInterface::ID => $manifestationId,
                '_current' => true,
            ]);

    ...
    public function getData()
    {
        ...
        if ('manifestation_form_data_source' === $this->name) {
            ...
            if ($data['totalRecords'] > 0) {
                $manifestationId = $data['items'][0][ManifestationInterface::MANIFESTATION_ID];
            ...
            if ($data['totalRecords'] > 0) {
                foreach ($data['items'] as $index => $manifestation) {
                    $manifestation_id = +$manifestation[ManifestationInterface::MANIFESTATION_ID];
                    $data['items'][$index]['assigned_places'] = $this->getAssignedPlacesById($manifestation_id);
                }
            }
        }

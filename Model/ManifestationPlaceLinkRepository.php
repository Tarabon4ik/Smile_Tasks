<?php
/**
 * Repository ManifestationPlaceLink
 *
 * @category  Smile
...
class ManifestationPlaceLinkRepository implements ManifestationPlaceLinkRepositoryInterface
{
    ...
    public function getByPlaceId($placeId): ManifestationPlaceLinkInterface
    {
        /** @var ManifestationPlaceLink $link */
        $link = $this->manifestationPlaceLinkFactory->create();
        $this->manifestationPlaceLinkResource->load($link, $placeId, ManifestationPlaceLinkInterface::PLACE_ID);
        if (!$link->getId()) {
            throw new NoSuchEntityException(__('No such link with placeId %1 !', $placeId));
        }
    ...
    public function getByManifestationId($manifestationId): ManifestationPlaceLinkInterface
    {
        /** @var ManifestationPlaceLink $link */
        $link = $this->manifestationPlaceLinkFactory->create();
        $this->manifestationPlaceLinkResource->load($link, $manifestationId, ManifestationPlaceLinkInterface::MANIFESTATION_ID);
        if (!$link->getId()) {
            throw new NoSuchEntityException(__('No such link with manifestationId %1 !', $manifestationId));
        }
    ...
    public function getList(SearchCriteriaInterface $searchCriteria = null): ManifestationPlaceLinkSearchResultsInterface
    {
        ...
        /** @var ManifestationPlaceLinkSearchResultsInterface $searchResults */
